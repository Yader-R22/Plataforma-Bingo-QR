...
function formatUser(user: typeof usersTable.$inferSelect) {
  return {
    ...
    has_id_photos: !!(user.idPhotoFrontUrl && user.idPhotoBackUrl),
...
router.post("/register", async (req, res) => {
  ...
  db.insert(feedItemsTable).values({
    type: "new_user",
    message: `${displayName}${dept ? ` de ${dept}` : ""} se unió a {SITE_NAME}`,
...
router.get("/me/stats", requireAuth, async (req: AuthRequest, res) => {
  ...
  }).from(winnersTable).where(and(eq(winnersTable.userId, req.userId!), eq(winnersTable.validated, true)));
  res.json({
