      ...
      if (res.ok) {
        ...
        setSession(prev => {
          if (prev && data.last_called_number && data.last_called_number !== prev.last_called_number) {
            ...
            setTimeout(() => setNewNumberAlert(null), 3000);
          }
          return data;
        });
      }
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "#0d0028" }}>
      ...
      {newNumberAlert && (
        ...
        >
          🎱 ¡Nuevo número cantado: <span className="text-xl">{newNumberAlert}</span>!
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-3">
        ...
        <div className="flex items-center gap-4">
          <div className="text-center">
            <p className="text-white/40 text-xs mb-1">Último número</p>
            {session?.last_called_number ? (
              <div
                className="number-ball w-16 h-16 text-2xl"
                style={{
                  background: `linear-gradient(135deg, hsl(258 72% 35%), hsl(280 60% 50%))`,
              ...
              <div className="w-16 h-16 rounded-full flex items-center justify-center text-white/30 font-black text-xl" style={{ background: "rgba(255,255,255,0.06)" }}>
                —
              </div>
            )}
          </div>
          ...
          <div className="flex-1">
            <p className="text-white/40 text-xs mb-1.5">Últimos cantados</p>
            <div className="flex flex-wrap gap-1.5">
              {(session?.called_numbers ?? []).slice(-9).reverse().map((n, i) => (
                <div
                  ...
                  style={{
                    background: i === 0 ? "hsl(42 98% 52%)" : "rgba(255,255,255,0.12)",
                    color: i === 0 ? "#1a0050" : "rgba(255,255,255,0.7)",
                  }}
                >
                  {n}
                </div>
        ...
        {showAllNumbers && session && session.called_numbers.length > 0 && (
          <div className="rounded-2xl p-3" style={{ background: "rgba(255,255,255,0.05)" }}>
            <p className="text-white/40 text-xs mb-2">Todos los números cantados ({session.called_numbers.length})</p>
            <div className="flex flex-wrap gap-1.5">
              {session.called_numbers.map(n => (
                <div key={n} className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: "rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.7)" }}>
                  {n}
                </div>
              ))}
            </div>
        ...
        {cards.length === 0 ? (
          ...
          <>
            ...
            <div className="rounded-2xl overflow-hidden shadow-2xl" style={{ border: "2px solid rgba(255,255,255,0.15)" }}>
              ...
              <div className="grid grid-cols-5">
                {LETTERS.map((l, i) => (
                  <div
                    key={l}
                    className="py-2.5 text-center font-black text-lg"
            ...
            <p className="text-center text-white/40 text-xs">
              Toca los números para marcarlos · {markedSet.size - 1} marcados
            </p>
          </>
        ) : null}
      </div>

      {/* BINGO button — fixed at bottom */}
      {card && (
        <div className="shrink-0 px-4 pb-6 pt-3" style={{ borderTop: "1px solid rgba(255,255,255,0.08)" }}>
          <button
            className="bingo-btn w-full h-16 rounded-2xl text-2xl disabled:opacity-40 disabled:cursor-not-allowed"
            onClick={claimBingo}
            disabled={claiming}
          >
            {claiming ? (
          ...
          <p className="text-center text-white/30 text-xs mt-2">
            Solo presiona cuando completes el patrón ganador
          </p>
        </div>
      )}
    </div>
