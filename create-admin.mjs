 ...
 *
 * Requiere que DATABASE_URL esté definido en el archivo .env del proyecto.
 */
...
function loadEnv() {
  try {
    ...
    console.error("❌  No se encontró el archivo .env. Asegurate de estar en /var/www/tubingazo");
    process.exit(1);
  }
}
...
let bcrypt, pg;
try {
  bcrypt = require("./artifacts/api-server/node_modules/bcryptjs");
} catch {
  try {
    bcrypt = require("bcryptjs");
  } catch {
    console.error("❌  No se encontró bcryptjs. Ejecutá: pnpm install");
    process.exit(1);
  }
}
try {
  pg = require("./node_modules/pg") ?? require("pg");
} catch {
  try {
...
let client;
try {
  const { Client } = pg?.Client ? pg : { Client: pg };
  client = new Client({ connectionString: process.env.DATABASE_URL });
  await client.connect();
} catch (err) {
  // Intentar con import dinámico si require falló
  try {
    ...
    console.error("❌  No se pudo conectar a la base de datos:", err2.message);
...
const exists = await client.query("SELECT id FROM users WHERE ci = $1", [ci.trim()]);
...
try {
  await client.query(
    ...
    [nombre.trim(), ci.trim(), telefono.trim(), passwordHash, departamento]
  );
} catch (err) {
...
console.log("\n  Podés entrar en: https://TU_DOMINIO.COM");
console.log("  Panel admin:    https://TU_DOMINIO.COM/admin\n");
